...
for i=1:size(cluster_median,1)
    [Y] = sort(cluster_median(i,:),'descend');
    cluster_median(i,cluster_median(i,:)>=Y(3))=Y(3);
end
...
for k=1:length(functional_ind)
    ...
    for i=1:length(case_files)
        ...
        [coeff] = draw_on_Axes_mst_here2(adj,coeff,max_diff,node_size,stim{case_files(i)},diff_signal(i,:),template);
        h4 = figure(4); 
        [coeff] = draw_on_Axes_mst_here2(adj,coeff,max_diff,node_size,[marker_names{functional_ind(k)}, ' ratio ',stim{case_files(i)},' vs ref'],diff_signal(i,:),template);
