...
for k=1:length(ind)  % for each markre to draw
    ...
    for i=1:length(case_files)
        ...
        if exist('is_universal_scale')  && is_universal_scale==1
            ...
            print(h5,['figs\Figures_RatioTrees_UniScale\',marker_name_tmp,'\',marker_name_tmp,'_',remove_bad_filename_letters(file_annot{case_files(i)}),'vsRefRatio.jpg'],'-djpeg');
            print(h5,['figs\Figures_RatioTrees_UniScale\all_RatioTrees\',marker_name_tmp,'_',remove_bad_filename_letters(file_annot{case_files(i)}),'vsRefRatio.jpg'],'-djpeg');
        else
    ...
    if exist('is_universal_scale')  && is_universal_scale==1
        print(h4,['figs\Figures_RatioTrees_UniScale\all_RatioTrees\',marker_name_tmp,'_AllStimvsRefRatio','.jpg'],'-djpeg')
        print(h4,['figs\Figures_RatioTrees_UniScale\Compact_subplots\',marker_name_tmp,'_AllStimvsRefRatio','.jpg'],'-djpeg')
        print(h4,['figs\Figures_RatioTrees_UniScale\',marker_name_tmp,'\',marker_name_tmp,'_AllStimvsRefRatio','.jpg'],'-djpeg')
    else
