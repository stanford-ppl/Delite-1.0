...
object BinarizedGradientGrid {
  ...
  def apply(modelFilenames: Rep[Vector[String]]) = {
  ...
  def fillTemplateFromGradientImage(gradSummary: GrayscaleImage, xc: Int, yc: Int, r: Int, level: Int): BinarizedGradientTemplate = {
    val span = 2 * r
    val tpl = new BinarizedGradientTemplate(r, null, null, level, Vector[Int](span * span), IndexVector(), null, null, null)
...
class BinarizedGradientGrid(all_templates: Rep[Vector[(String, Vector[BinarizedGradientTemplate])]]) {
