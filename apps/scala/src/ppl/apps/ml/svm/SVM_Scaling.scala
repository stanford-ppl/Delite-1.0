...
object SVM_Scaling extends DeliteApplication {
  def print_usage = {
  ...
  def run(args: Array[String]) = {
    ...
    for (iter <- 0 until numTests) {
      ...
      val svm_test = new SVMModel_Scaling(modelFile)
