...
class SVMModel() {
  ...
  def train(X: Matrix[Int], Y: Vector[Int], C: Double, tol: Double, max_passes: Int) = {
    ...
    val alphas = Vector.zeros(X.numRows).trans // col vector
    ...
    while (passes < max_passes){
      ...
      for (i <- 0 until numSamples){
        val f_i = (alphas*Y*(X*X(i).trans)).sum[DeliteDouble] + b
        val E_i = f_i - Y(i)
