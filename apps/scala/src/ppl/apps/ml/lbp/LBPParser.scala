...
object LBPParser {
  ...
  def read_graph(inFile: File, max_node_num: Int, num_edges: Int)
    ...
    val nodes = Vector[Int](max_node_num+1)
    val l_edges = Vector[Int](num_edges)
    val r_edges = Vector[Int](num_edges)
   ...
   *
   * TODO: This should parse into a graph or factor graph.
   *       create data types for Node and Edge instead of Vector[Int]
   *
   * @param inFile: File handle to be read
   * @param max_num_node
   * @param num_edges
  ...
  def read_graph_lr(inFile: File, max_node_num: Int, num_edges: Int)
    ...
    val nodes = Vector[Int](max_node_num+1)
    val edges = Matrix[Int](num_edges, 2)
   ...
   */
  def read_beliefs(inFile: File, nodes: Vector[BPNode], last_cat_node: Int, last_node: Int)
    : Vector[Double] = {
   ...
   /* printf("parse potentials for category nodes\n");*/
    ...
    val edge_potential = Vector[Double](CAT_BELIEF_SIZE * REL_BELIEF_SIZE)
  ...
  def print_beliefs(outFile: File, begin: Int, num_nodes: Int, nodes: Vector[BPNode]) = {
