...
abstract class DeliteOP_NativeMapSpec[@specialized(Double,Float,Int)A, C[X] <: DeliteCollection[X]] extends DeliteOP_Map[A, A, C] {

  val coll: DeliteCollection[A]
  val out: C[A]

  override def func: A => A
  def funcNative(start:Int, end:Int) : Unit
  ...
  private def specMapTask(coll: DeliteCollection[A], func: A => A, pos: Int, total: Int) {
	...
	funcNative(idx, end)
	//DeliteNative.vecSigmoidFloat(coll._data, out._data, idx, end)
  }
}