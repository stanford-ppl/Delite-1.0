...
class GraphOP_untilConvergedData[V: ClassManifest, E](g: Graph[V, E], coll: DeliteCollection[V], func: Graph[V, E]#Vertex => Unit, locks: ConcurrentHashMap[V, ReentrantLock]) extends DeliteOP[Vector[V]] {
  ...
  def task = {
    ...
    while (idx != end) {
      ...
      val vertices = g.fullVertices(vertex)
      g.lockVerticesFull(vertices, locks)
      func(vertex)
      g.unlockVerticesFull(vertices, locks)
