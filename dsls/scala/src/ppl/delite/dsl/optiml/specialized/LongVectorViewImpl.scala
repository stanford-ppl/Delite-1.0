...
class LongVectorViewImpl
  extends VectorView[Long] with LongVector with GPUable[Long]{

  type DSLType = LongVectorViewImpl

  def this(x: Array[Long], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    this()
    init(x, offset, str, len, row_vec)
    isComputed = true
  }

  def init(x: Array[Long], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    _data = x
    _start = offset
    _stride = str
    _length = len
  ...
  def mutableClone = Vector[Long](is_row, 0) ++= this
