...
class IntVectorViewImpl
  extends VectorView[Int] with IntVector with GPUable[Int]{

  type DSLType = IntVectorViewImpl

  def this(x: Array[Int], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    this()
    init(x, offset, str, len, row_vec)
    isComputed = true
  }

  def init(x: Array[Int], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    _data = x
    _start = offset
    _stride = str
    _length = len
  ...
  def mutableClone = Vector[Int](is_row, 0) ++= this
