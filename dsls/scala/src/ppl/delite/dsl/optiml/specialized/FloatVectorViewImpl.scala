...
class FloatVectorViewImpl
  extends VectorView[Float] with FloatVector with GPUable[Float]{

  type DSLType = FloatVectorViewImpl

  def this(x: Array[Float], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    this()
    init(x, offset, str, len, row_vec)
    isComputed = true
  }

  def init(x: Array[Float], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    _data = x
    _start = offset
    _stride = str
    _length = len
  ...
  def mutableClone = Vector[Float](is_row, 0) ++= this
