...
class DoubleVectorViewImpl
  extends VectorView[Double] with DoubleVector with GPUable[Double]{

  type DSLType = DoubleVectorViewImpl

  def this(x: Array[Double], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    this()
    init(x, offset, str, len, row_vec)
    isComputed = true
  }

  def init(x: Array[Double], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    _data = x
    _start = offset
    _stride = str
    _length = len
  ...
  def mutableClone = Vector[Double](is_row, 0) ++= this
