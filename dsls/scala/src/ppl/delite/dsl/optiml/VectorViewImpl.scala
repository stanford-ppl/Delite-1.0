...
class VectorViewImpl[T : ClassManifest] extends VectorView[T]{

  type DSLType = VectorViewImpl[T]

  def this(x: Array[T], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    this()
    init(x, offset, str, len, row_vec)
    isComputed = true
  }

  def init(x: Array[T], offset: Int, str: Int, len: Int, row_vec: Boolean) = {
    _data = x
    _start = offset
    _stride = str
    _length = len
  ...
  def mutableClone = Vector[T](is_row, 0) ++= this
  ...
  def lifted_apply(n: Int) : T = apply(n)
  def apply(n: Int) : T = {
    _data(chkIndex(idx(n)))
  }

  def lifted_update(n: Int, x: T) = update(n,x)
